 ...
 * 
 * To compile:
 *   mpicc -o Linear_Regression_MPI Linear_Regression_MPI.c -lm
 * 
 * To run:
 *   mpirun -n 9 ./Linear_Regression_MPI
 *****************************************************************************/
...
double ResError(double a, double b, double m, double c) {
  double e = (m * a) + c - b;
  return e * e;
}
...
double rms_error(double m, double c) {
  ...
  double error_sum = 0;
  for(i=0; i<n_data; i++) {
    error_sum += ResError(data[i].a, data[i].b, m, c);  
  }  
...
int main() {
  ...
  double process_e;
  while(!minimum_found) {
    if(rank ==0){   // action processes
      for(i=0;i<8;i++) {
        dm[i] = bm + (om[i] * step);
        dc[i] = bc + (oc[i] * step);    
        // send dm and dc to each process
        MPI_Send(&dm, 8, MPI_DOUBLE, i+1, 0, MPI_COMM_WORLD);
        MPI_Send(&dc, 8, MPI_DOUBLE, i+1, 0, MPI_COMM_WORLD);
      }
      // receive process results and work on them
      for(i=0;i<8;i++){
        MPI_Recv(&process_e, 1, MPI_DOUBLE, i+1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        e[i] = process_e; // put segment result in error array
      ...
      if(best_error < be) {
        ...
        bm = dm[best_error_i];
        bc = dc[best_error_i];
        //printf("best_error = %lf, be = %lf\n", best_error, be);
      } else {
        minimum_found = 1;
      }
      for(i=0;i<8;i++) { // notify other processes to recalculate or finalize
        MPI_Send(&minimum_found, 1, MPI_INT, i+1, 0, MPI_COMM_WORLD);

      }
    }
    if(rank !=0){  // run process
      // receive m and c values to process from rank 0
      MPI_Recv(&dm, 8, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Recv(&dc, 8, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      // complete process and send  result back to rank 0
      process_e = rms_error(dm[rank-1], dc[rank-1]);
      MPI_Send(&process_e, 1, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
      // discover if process needs to terminate (rank 0 sends 1)
